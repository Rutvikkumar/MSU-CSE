...
const MessageCard = ({ title, imageSrc, name, position, message, readMoreLink }) => {
  return (
    <div className="overflow-hidden rounded-2xl shadow-lg border border-gray-300 hover:shadow-2xl transition-shadow duration-300 bg-white h-full flex flex-col">
      {/* Header */}
      <div className="bg-gradient-to-r from-sky-950 to-teal-700 p-5 text-white text-center rounded-t-2xl">
        <h2 className="text-2xl font-bold">{title}</h2>
      </div>
...
const LeadershipCards = () => {
  const cards = [
    {
      title: "Chairman's Message",
      imageSrc: messages.chairman.image,
      name: messages.chairman.name,
      position: messages.chairman.title,
      message: messages.chairman.message,
      readMoreLink: "/about/hod-message"
    },
    {
      title: "Principal's Message",
      imageSrc: messages.principal.image,
      name: messages.principal.name,
      position: messages.principal.title,
      message: messages.principal.message,
      readMoreLink: "/about/hod-message"
    },
    {
      title: "HOD's Message",
      imageSrc: messages.hod.image,
      name: messages.hod.name,
      position: messages.hod.title,
      message: messages.hod.message,
      readMoreLink: "/about/hod-message"
    }
  ];
  ...
  return (
    <div className="container mx-auto px-4 py-12 bg-blue-50">
      <h1 className="text-3xl font-bold text-center mb-12 text-gray-800">Leadership Messages</h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {cards.map((card, index) => (
          <MessageCard key={index} {...card} />
        ))}
